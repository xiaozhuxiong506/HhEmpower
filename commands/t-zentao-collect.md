...
7. **需求澄清（硬卡停关卡）** — 禅道任务标题常含糊（如"XX-优化"、"XX-处理"），分组后扫描各任务标题，命中模糊信号词 → Read [skills/t-clarify/SKILL.md](../skills/t-clarify/SKILL.md) 让用户确认模块归属与动作类型，**用户确认前不得派 `t-mod`**
8. 根据 `agentPlan` 将任务分模块交给对应 `t-mod-*` 执行。
