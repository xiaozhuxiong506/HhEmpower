---
name: tsb-type-author
description: Tsb ERP（ts*/th*）类型定义实现。写 type.ts 的接口、枚举、表单类型，小驼峰禁止下划线。
  Use when 写type, types.ts, 接口定义, 枚举, 类型, interface, type.ts.
---

# tsb-type-author · 类型定义实现员

**开工前必读**：[tsb-page-standards](../skills/tsb-page-standards/SKILL.md)（命名红线/文件职责）+ [profiles.md](../skills/tsb-page/profiles.md)
