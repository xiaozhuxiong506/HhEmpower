...
> 总入口：[tsb-page](../tsb-page/SKILL.md) · 画像：[profiles.md](../tsb-page/profiles.md) · 锚点：[anchors.md](../tsb-page/anchors.md) · 细则：[reference.md](reference.md)

## 第 0 步：探测项目

任何工作流开始前先读 [profiles.md](../tsb-page/profiles.md)，判定当前是 ts（TradeErp）还是 th（HallErp），后续 `${占位符}` 套用对应列。
...
- 意图不清、需求多义、技术方案多条路径
- **主单 vs 详情范围不清** → [anchors.md#主单--详情映射](../tsb-page/anchors.md#主单--详情映射)
- 删/改/加功能，且主单与详情（或同页多入口）均可能存在
- 弹窗/抽屉/表单**新增字段** → [tsb-page-field-setting](../tsb-page-field-setting/SKILL.md)
- Tab/弹窗**新增改数据或确认按钮** → [tsb-page-tab-editability](../tsb-page-tab-editability/SKILL.md)
...
1. 找根因，不只改表象
2. Grep 同函数/字段/API/**兄弟页与 Tab 子组件**（见 tsb-page-registry）
...
3. 五处 name 一致 · OP_HEADERS · hasAuth · virtualYConfig
4. **非必须不改 locales**；新增 key → 总表 #4 → #5
...
| 场景 | 处理 |
|------|------|
| 新增 OP_HEADERS | 总表 #6 → [tsb-page-operation-log](../tsb-page-operation-log/SKILL.md) |
| 多文件可复用写法 | 总表 #7 问是否抽离；默认 **不抽离** |
| 多文件同类修改 | **禁止脚本**：跨模块按菜单拆 `tsb-module-agent` 实例并行；单模块逐文件 AI 改 |

## 最简代码（必守）

只改当前问题所需 · 照抄标准页一小段 · 不新造 hook/工具层 · 冲突时 **以更简单为准**

编码克制 → [tsb-page-standards](../tsb-page-standards/SKILL.md) · 命名红线：禁止下划线，一律小驼峰
