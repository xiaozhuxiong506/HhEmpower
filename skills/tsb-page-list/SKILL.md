...
const {
  ...
  sysRoleFiledCode: "${Views大写}Xxx",   // 五处一致之一
...
**Tab 可编辑性**：改 Tab 或工具栏按钮前，梳理各 `activeTab` 的显隐与 `disabled`；只读 Tab 不加保存类操作 → [tsb-page-tab-editability](../tsb-page-tab-editability/SKILL.md)
...
业务列 < 9 → 不设 width；>= 9 → 按 tsb-page-standards 通用值设 width。
禁止 `minWidth`（含 columnConfig.minWidth）。详见 [tsb-page-standards/reference.md](../tsb-page-standards/reference.md#列宽与-minwidth)。
...
- import 顺序不符 tsb-page-standards
- 新增列表列未问字段设置（总表 #13 含 D 列表可见）→ [tsb-page-field-setting](../tsb-page-field-setting/SKILL.md)
- 参考他模块复用 `t('key')` 不查 locales → [tsb-page-workflow/reference.md](../tsb-page-workflow/reference.md#参考他模块时的-i18n)
- 用户说主单却改详情，或范围模糊未问单选题（总表 #2）→ [anchors.md](../tsb-page/anchors.md)

必问单选题统一规则 → [tsb-page-workflow/reference.md](../tsb-page-workflow/reference.md#必问单选题总表)

关联：[tsb-page-workflow](../tsb-page-workflow/SKILL.md) · [tsb-page-detail](../tsb-page-detail/SKILL.md) · [tsb-page-auth](../tsb-page-auth/SKILL.md) · [tsb-page-operation-log](../tsb-page-operation-log/SKILL.md)
